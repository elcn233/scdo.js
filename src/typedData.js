const { id, keccak256, concat, defineProperties, getBigInt, getBytes, hexlify, mask, toTwos, toBeHex, zeroPadValue, assertArgument } = require("ethers");
...
function getBaseEncoder(type) {
    ...
    switch (type) {
        case "srcToken": return getBaseEncoder('uint256');
...
class TypedDataEncoder {
    ...
    constructor(types) {
        ...
        for (const [name, set] of subtypes) {
            ...
            st.sort();
            this.privateFullTypes.set(name, encodeType(name, types[name]) + st.map((t) => encodeType(t, types[t])).join(""));
        }
    }
    getEncoder(type) {
        let encoder = this.privateEncoderCache.get(type);
        if (!encoder) {
    ...
    privateGetEncoder(type) {
        ...
        if (match) {
            ...
            return (value) => {
                ...
                if (this.privateFullTypes.has(subtype)) {
        ...
        if (fields) {
            const encodedType = id(this.privateFullTypes.get(type));
            return (value) => {
                const values = fields.map(({ name, type }) => {
                    const result = this.getEncoder(type)(value[name]);
                    if (this.privateFullTypes.has(type)) {
    ...
    encodeType(name) {
        const result = this.privateFullTypes.get(name);
