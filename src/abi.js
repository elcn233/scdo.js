...
const { address, ADDRESS_PREFIX, ADDRESS_PREFIX_REGEX } = require('./address');
...
function encodeParamsV2ByABI(funABI, args) {
    ...
    const buildFullTypeDefinition = (typeDef) => {
        ...
        if (/srcToken/.test(typeDef.type))
            return typeDef.type.replace(/srcToken/, 'uint256');
    ...
    const convertTypes = (types) => {
        for (let i = 0; i < types.length; i++) {
            const type = types[i];
            if (/srcToken/.test(type))
                types[i] = type.replace(/srcToken/, 'uint256');
        }
    };
    ...
    const iface = new Interface([funABI]);
    const abiData = iface.encodeFunctionData(funABI.name, args);
    return abiData;
}
...
function decodeParamsV2ByABI(funABI, data) {
    ...
    const buildFullTypeNameDefinition = (typeDef) => {
        ...
        if (/srcToken/.test(typeDef.type))
            return typeDef.type.replace(/srcToken/, 'uint256') + name;
