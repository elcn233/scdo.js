...
class Scdo {
  constructor(host, headers, user, password, timeout) {
    ...
    this.client = new ScdoWebProvider(host,headers,user,password,timeout);
  }
}

class ScdoKeyActions {
...
/**
* ScdoWebProvider should be used to send rpc calls over http
...
for (const namespace in api.commands) {
  api.commands[namespace].forEach(command => {
    var cp = ScdoWebProvider.prototype
    cp[command] = function() {
